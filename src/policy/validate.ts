...
function isDenied(denyRead: string[], targetPath: string): boolean {
  return denyRead.some(p => p === targetPath || isInside(p, targetPath));
}
...
export function validate(fragment: PolicyFragment, granted: Set<UnsafeOverride>): void {
  ...
  if (!granted.has('host-claude-home') && !isDenied(denyRead, claudePath)) {
